...
function createItem(id, text) {
    ...
    // Agregar eventos drag al nuevo item
    item.addEventListener('dragstart', handleDragStart);
    item.addEventListener('dragend', handleDragEnd);
 ...
 */
function resetOrder() {
    if (confirm('¿Estás seguro de que quieres resetear todas las tareas al orden inicial?')) {
...
function addTask() {
    ...
    if (text) {
        ...
        const newItem = createItem(nextId++, text);
        todoZone.appendChild(newItem);
        saveState();
        hideAddTaskModal();
        
        // Animación de entrada para el nuevo item
...
console.log('🎯 Drag and Drop inicializado con modal');